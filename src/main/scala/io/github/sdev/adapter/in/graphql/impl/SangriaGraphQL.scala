...
object SangriaGraphQL {
  ...
  def apply[F[_]: Async, Applicative, A](
    ...
    override def query(query: String, operationName: Option[String], variables: JsonObject): F[Either[Json, Json]] =
      QueryParser.parse(query) match {
        case Success(ast)                       => exec(schema, ast, operationName, variables)(blockingExecutionContext)
        case Failure(e @ SyntaxError(_, _, pe)) => fail(formatSyntaxError(e))
        case Failure(e)                         => fail(formatThrowable(e))
      }

    def fail(json: Json): F[Either[Json, Json]] =
      json.asLeft.pure[F]

    def exec(schema: Schema[A, Unit], query: Document, operationName: Option[String], variables: JsonObject)(implicit
        ec: ExecutionContext
    ): F[Either[Json, Json]] =
      userContext
        .flatMap { ctx =>
          Async[F].delay { (cb: Either[Throwable, Json] => Unit) =>
            Executor
              .execute(
                schema = schema,
